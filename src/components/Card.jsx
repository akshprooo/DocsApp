...
const Card = ({things, reference}) => {
  return (
    <motion.div drag dragConstraints={reference} whileDrag={{scale: 1.2}} className='relative w-60 h-80 p-5 rounded-[20px] bg-zinc-900/90 text-white flex flex-col gap-6'>
        ...
        <div className='flex items-center px-5 absolute bottom-0 left-0 rounded-b-[20px] w-full h-15' style={{backgroundColor: !things.isDownload ? `${things.bgcolor}` : '', justifyContent: !things.isDownload?'center':'space-between'}}>
            {!things.isDownload?
            <>
            ...
            :
            <>
                <h1 className=''>{things.dataAmount}</h1>
